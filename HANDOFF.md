...
- **设置信息架构**（按账户拆分）：**广告账户**（真实连接列表：Meta 连接/重新授权/断开，其余平台诚实标「后端未接入」，假示例已删）· **LLM 接口**（独立页：提供方胶囊 DeepSeek/Kimi/通义/智谱/OpenRouter/自定义 → 自动填 base+取 key 链接；模型下拉可「↻ 拉取列表」从该家 `/models` 实时拉（五家 CORS 已验证全开）；「测试连接」发 1-token chat 报可用/人话错误；随账户同步）· **运行设置**（模式/预算上限/目标与预算/归因）· **账户**（身份 + 同步状态）。
